...
- Fed validated `IBG1/F1-F5` into the arrangement as `RCB1`. Result: strong recovered-contact candidate, but blocked as `same_pair_line_alias`: it has `0.0` atom IoU with raw same-pair regions, while nearest raw `F1-F5` region `OB2` has fitted-line distance `1.03 mm` and candidate-axis span overlap `0.486`.
- Visual verdict: `RCB1` appears above the raw same-pair band and is plausibly related to the missed middle bend, but same-pair line deconfliction says it is too close to `OB2` in line space to count separately.
- Decision: do not promote. Keep `48991006` as a poor-scan / duplicate-support / missed-interior-bend fail-safe case. The next step is same-pair support consolidation/repair for the `F1-F5` family, not raw birth promotion.
