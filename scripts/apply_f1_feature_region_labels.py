...
from summarize_f1_feature_region_labels import VALID_LABELS, summarize_feature_region_labels
...
def _parse_label_assignment(raw: str) -> tuple[str, str]:
    if "=" not in raw:
        raise ValueError(f"label assignment must be BEND_ID=LABEL, got {raw!r}")
    ...
    if label not in VALID_LABELS:
        raise ValueError(f"invalid label {label!r} for {bend_id}; valid labels: {sorted(VALID_LABELS)}")
...
def _load_label_map(path: Optional[Path]) -> Dict[str, str]:
    ...
    if not isinstance(raw_map, Mapping):
        raise ValueError("label map JSON must be an object or contain an object at key 'labels'")
    labels: Dict[str, str] = {}
    for bend_id, label in raw_map.items():
        bend = str(bend_id).strip()
        value = str(label).strip()
        if value not in VALID_LABELS:
            raise ValueError(f"invalid label {value!r} for {bend}; valid labels: {sorted(VALID_LABELS)}")
        labels[bend] = value
    return labels


def apply_feature_region_labels(
    *,
    ...
    known_regions = {str(row.get("bend_id") or "") for row in list(payload.get("region_labels") or ())}
    unknown = sorted(set(label_map) - known_regions)
    if unknown:
        raise ValueError(f"unknown bend ids in label map: {unknown}")
    for row in list(payload.get("region_labels") or ()):
        bend_id = str(row.get("bend_id") or "")
        if bend_id in label_map:
            row["human_feature_label"] = str(label_map[bend_id])
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Apply simple OB label assignments to an F1 feature-region label template.")
    parser.add_argument("--template-json", required=True, type=Path)
    parser.add_argument("--label-map-json", type=Path, default=None, help="JSON object like {'OB1': 'conventional_bend'} or {'labels': {...}}.")
    parser.add_argument("--label", action="append", default=[], help="Inline assignment, e.g. OB1=conventional_bend. May be repeated.")
    ...
    filled = apply_feature_region_labels(template_payload=_load_json(args.template_json), label_map=labels)
    _write_json(args.output_json, filled)
    summary = summarize_feature_region_labels(filled)
