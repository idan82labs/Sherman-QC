...
# CORS configuration. Wildcard CORS is disabled by default because this API can
# trigger AI providers and return proprietary manual evidence.
...
PRODUCTION_ENV = os.environ.get("PRODUCTION", "").strip().lower() in {"1", "true", "yes", "on"}
if CORS_ORIGINS_ENV and CORS_ORIGINS_ENV.strip() != "*":
    ALLOWED_ORIGINS = [origin.strip() for origin in CORS_ORIGINS_ENV.split(",") if origin.strip()]
elif CORS_ORIGINS_ENV.strip() == "*" and os.environ.get("ALLOW_WILDCARD_CORS", "").strip().lower() == "true" and not PRODUCTION_ENV:
    ALLOWED_ORIGINS = ["*"]
else:
...
def _run_analysis_worker_subprocess(
    ...
    existing_pythonpath = worker_env.get("PYTHONPATH", "").strip()
    worker_paths = [str(BASE_DIR), str(BASE_DIR / "backend")]
    if existing_pythonpath:
        worker_paths.append(existing_pythonpath)
    worker_env["PYTHONPATH"] = os.pathsep.join(worker_paths)
