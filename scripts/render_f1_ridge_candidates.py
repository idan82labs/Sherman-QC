...
def render_ridge_candidates(
    *,
    ...
    selected = list(ridges.get("ridges") or [])[:top_n]
    ...
    for index, (title, view) in enumerate(views, start=1):
        ...
        for ridge_index, ridge in enumerate(selected):
            color = colors[ridge_index % len(colors)]
            atom_ids = {str(value) for value in ridge.get("atom_ids") or ()}
            label = f"{ridge.get('ridge_id')} atoms={ridge.get('atom_count')} score={ridge.get('mean_ridge_score')}"
            _scatter(ax, _points_for_atoms(atom_ids, atoms), color=color, size=13, alpha=0.68, label=label)
    ...
    fig.suptitle(f"{decomposition.get('part_id')} scan-wide ridge candidate diagnostic", fontsize=15, fontweight="bold", y=0.985)
    fig.tight_layout(rect=(0, 0.08, 1, 0.955), pad=0.25)
    output_path = output_dir / "ridge_candidates_top_1080p.png"
    ...
    sidecar = {
        ...
        "ridges_json": str(ridges_json),
        "top_ridge_ids": [ridge.get("ridge_id") for ridge in selected],
    }
    (output_dir / "ridge_candidates_render_sidecar.json").write_text(json.dumps(sidecar, indent=2), encoding="utf-8")
    return sidecar


def main() -> int:
    parser = argparse.ArgumentParser(description="Render top F1 ridge candidates for visual support-discovery QA.")
    parser.add_argument("--decomposition-json", required=True, type=Path)
    parser.add_argument("--ridges-json", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--top-n", type=int, default=6)
    args = parser.parse_args()
    sidecar = render_ridge_candidates(
        decomposition_json=args.decomposition_json.expanduser().resolve(),
        ridges_json=args.ridges_json.expanduser().resolve(),
